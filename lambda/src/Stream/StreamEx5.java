...
// 최종 연산
// count(), min(), max(), average(), findFirst(), sum()
...
//		stream1.filter(i -> i % 2 == 0).forEach(System.out::println);
		System.out.println("2의 배수 갯수 : " + stream1.filter(i -> i % 2 == 0).count());
		...
		stream1 = Arrays.stream(new int[] {1,4,6,8,9});
		System.out.println("2의 배수 첫 번째값 : " + stream1.filter(i -> i % 2 == 0).findFirst());
