...
//		System.out.println("[공통로그] 비즈니스 로직 수행 후 호출(예외 발생 여부와 관계 없이 무조건 실행)");
//	}
	
//	@AfterThrowing(value = "execution(* com.spring.aop.Product.getInfo())")
//	public void afterThrowingLog() {		
//		System.out.println("[공통로그] 비즈니스 로직 수행 후 예외 발생 시 실행");
//	}
	
//	@AfterReturning(value = "execution(* com.spring.aop.Product.getInfo())")
//	public void afterReturningLog() {		
//		System.out.println("[공통로그] 비즈니스 로직 수행 후 예외 발생이 없을 시 실행");
//	}
	
	@Around(value = "execution(* com.spring.aop.Product.getInfo())")
	public void aroundLog(ProceedingJoinPoint pjp) {		
		System.out.println("[공통로그] 비즈니스 로직 수행 전 실행");
		try {
			pjp.proceed();
		} catch (Throwable e) {
			e.printStackTrace();
		}
		System.out.println("[공통로그] 비즈니스 로직 수행 후 실행");
	}
